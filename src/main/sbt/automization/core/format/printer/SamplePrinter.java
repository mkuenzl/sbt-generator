...
public final class SamplePrinter
        implements TextPrinter
{
    ...
    public double measureThickness(Sample sample)
    {
        ...
        {
            String formattedValue = sample.get(SampleKey.THICKNESS)
                                          .replace(",", ".");
    ...
    public String printSamplesMaterials(DataTable dataTable)
    {
		...
		List<Sample> outcropLayerSamples = sampleFormatter.combineSamplesByTag(SampleKey.TYPE, SampleKey
		.ROUNDING_GRADATION, SampleKey.GRANULATION);
		*/
    ...
    public String printAttributeOfSamplesWithDepth(final DataTable dataTable, final String outcrop, final Key tag)
    {
        ...
        for (int i = 0; i < amountOfSamples; i++)
        {
            ...
            if (tag == ProbeKey.WEAR_TRENCH_BOTTOM)
            {
                final Probe probe = sample.getProbe();
                String depth = probe.get(ProbeKey.SOLE_DEPTH);
                stringBuilder.append(new DepthTextFormatter(true).formatSingleDepth(depth));
...
//					sample.get(SampleKey.DEPTH_END)));
    ...
    public String printAttributeOfDatatable(final DataTable dataTable, final Key tag)
    {
        ...
        {
            formattedTag =
                    new ChemistryMarkupTextFormatter().format(dataTable.getParameterValueBy(SampleKey.CHEMISTRY_ID,
                                                                                            tag));
        }
        else if (tag instanceof RuKKey)
        {
            formattedTag = new HtmlText.Builder()
                    ...
                    .appendContent(new StandardCellTextFormatter().format(dataTable.getParameterValueBy(SampleKey.RUK_ID,
                                                                                                        tag)))
                    .build()
                    .appendTag();
        }
        else if (tag == SampleKey.MOISTURE)
        {
            formattedTag = new HtmlText.Builder()
                    .appendAttribute("class", "Normal")
                    .appendContent(new StandardCellTextFormatter().format(new ProctorTextFormatter().format(dataTable.get(
                            tag))))
                    .build()
                    .appendTag();
        }
        else if (tag == ProbeKey.WEAR_TRENCH_BOTTOM)
        {
            final Probe probe = ((Sample) dataTable).getProbe();
            formattedTag = new HtmlText.Builder()
                    .appendAttribute("class", "Normal")
                    .appendContent(new StandardCellTextFormatter().format((probe.get(tag))))
                    .build()
                    .appendTag();
        }
        else
